  ...
  implements GGButtonListener
{
  ...
  private DieController dieController = new DieController(this);
  ...
  void prepareRoll(int currentIndex)
  {
    ...
    else
    {
      ...
      int currentPuppet = gp.getCurrentPuppetIndex();
      // 判断有没有相同的地方
      int i = 0;
      for(Puppet pp : gp.getAllPuppets()){
        if (i != currentPuppet && pp.getCellIndex() == currentIndex && !pp.getIsOverlap()){
            pp.go(-1);
            pp.setOverlap(true);
//            if(currentPuppet > 0){
//              gp.setCurrentPuppetIndex(currentPuppet - 1);
//            }else if(currentPuppet == 0) {
//              gp.setCurrentPuppetIndex(gp.getNumberOfPlayers() - 1);
//            }
        }
        i++;
      }
  ...
  private void roll(int rollNumber)
  {
    ...
    dieController.roll(nb);
    addActor(dieController.LastDie(), dieBoardLocation);
  }
